...
<body ">
	...
	<div class="content container"  style="padding-top:30px; border: rebeccapurple; border-style: dotted; padding:50px">
 		 <a href="createtrinhdochinhtri">Thêm mới</a>
 		 
 		 <table class="table table-hover table-striped">
 		 	<thead>
		 		 <tr>
		 			<th>STT</th>
		 		 	<th> Mã trình độ chính trị </th>
		 		 	<th> Tên trình độ chính trị</th> 
		 		 	<th></th> 
		 		 </tr>
	 		 </thead>
	 		 <tbody>
		 		 <c:forEach items="${listtrinhdochinhtri}" var="item" varStatus="loop"> 
					<tr>
						<td>${loop.index+1}</td>
			 		 	<td> ${item.maTrinhDoChinhTri} </td>
			 		 	<td> ${item.tenTrinhDoChinhTri} </td> 
			 		 	<td><a href="edittrinhdochinhtri/${item.maTrinhDoChinhTri}">Sửa</a></td> 
		 		 	</tr>
				 </c:forEach>
	 		  </tbody>
 		 </table>
